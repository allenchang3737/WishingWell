//
//  LUNRatingStarView.swift
//  TheWayToBasketball
//
//  Created by TWMP_IT_1 on 2019/7/25.
//  Copyright © 2019 LunYuChang. All rights reserved.
//

import UIKit

@objc protocol LUNRatingStarViewDelegate {
    //Returns the rating value when touch events end
    @objc optional func floatRatingView(_ ratingView: LUNRatingStarView, didUpdate rating: Double)
    
    //Returns the rating value as the user touching
    @objc optional func floatRatingView(_ ratingView: LUNRatingStarView, isUpdating rating: Double)
}

class LUNRatingStarView: UIView {
    ...
    var delegate: LUNRatingStarViewDelegate?
    ...
    var emptyImage: UIImage? {
        didSet {
            ...
            for item in emptyImageViews {
//                item.tintColor = .LunSeparatedColor()
                item.image = emptyImage//?.withRenderingMode(.alwaysTemplate)
            }
            refresh()
        }
    }
    ...
    // MARK: Type
    var type: RatingViewType = .wholeRatings
    
    enum RatingViewType: Int {
        // Integer rating
        case wholeRatings
